...
4. На разметке Yml и Ansible автоматизирован процесс получения сертификатов, как с помощью Letsencrypt (NBD: необходимо разделить по времени инсталляции pubkey и fullchain в конфиг виртуальной машины, на которой развернуты сразу три сервиса - prometheus, grafana, altermanager), так и с помощью Openssl для инвалидных сертификатов и ключей для обмена данными сервисов между собой.
