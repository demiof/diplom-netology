...
2. Пять виртуальных машин разработано для сервисов, указаных ниже - одна из них, с особенным образои NAT, для организации Reverse Proxy. Фактически данная машина так же может выполнять роль Bastion. Автоматизирован поцесс сборки и выгрузки конфигураций NGINX.

3. Используя возможности Packer, разработаны соответстующие образы (2-х типов), в которые "зашиты" необходимые "слои" и данные.

4. На Yml и Ansible автоматизирован процесс получения сертификатов, как с помощью Letsencrypt (NBD: необходимо разделить по времени инсталляции pubkey и fullchain в конфиг виртуальной машины, на которой развернуты сразу три сервиса - prometheus, grafana, altermanager), так и с помощью Openssl для инвалидных сертификатов и ключей для обмена данными сервисов между собой.
...
Так же автоматизирован способ получения CA сертификатов для доступа по https, как с подтверждением домена - для публичного доступа, так и самоподписных - для авторизации сервисов между собой по https.
...
Все sensitive и критичные данные спрятаны в переменные окружения ОС.
...
![Получение и отображение данных кластера 2-х нод БД - managed-mysql managed-mysql](assets/prometheus-mysql.jpg)